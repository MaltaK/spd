...
export default class Application extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            tableRows: props.session, // строки таблицы, которые могут быть отфильтрованы
            allRows: props.session, // полный список строк тадлицы
            tableSort: null, // сортировка в таблице
        };
        this.tableColumns = [
            { key: 'id', caption: '№', width: 60 },
            { key: 'objectID', caption: 'ID', width: 250},
            { key: 'name', caption: 'Имя пользователя', width: 250},
        ];
    }

    componentDidUpdate = prevProps => {
        if (prevProps.session !== this.props.session) {
            this.setState({ tableRows: this.props.session, allRows: this.props.session});
        }
    }
